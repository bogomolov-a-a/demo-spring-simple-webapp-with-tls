...
sudo "$JAVA_HOME"/bin/keytool -delete -noprompt -trustcacerts -alias rootCA -keystore "$JAVA_HOME"/lib/security/cacerts -storepass changeit
sudo "$JAVA_HOME"/bin/keytool -import -noprompt -trustcacerts -alias rootCA -file "$1"/rootCA.crt -keystore "$JAVA_HOME"/lib/security/cacerts -storepass changeit
