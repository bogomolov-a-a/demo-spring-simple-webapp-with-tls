echo 'start TLS key pair generating'
openssl genrsa -out "$1"/localhost.key "$2"
openssl req -new -config "$5"/openssl-server.conf -reqexts server_cert -subj "/CN=localhost/ST=St.Petersburg/C=RU/O=ZORN LEMMA/OU=develop" -key "$1"/localhost.key -out "$1"/localhost.csr
